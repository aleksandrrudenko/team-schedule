...
```
Замените `your-email@gmail.com` на **ваш реальный email**, который вы используете для входа в Google.

**Вариант 2: Несколько пользователей**
```
ALLOWED_USERS=user1@gmail.com,user2@gmail.com,admin@company.com
```
Просто перечислите email адреса через запятую.

### Примеры:

Если ваш email `ivan.petrov@gmail.com`:
```
ALLOWED_USERS=ivan.petrov@gmail.com
```

Если хотите разрешить доступ нескольким людям:
```
ALLOWED_USERS=ivan.petrov@gmail.com,maria.sidorova@gmail.com,admin@playson.com
```

### ⚠️ Важно:
- Используйте **тот же email**, который вы используете для входа в Google
- Email должен быть **точным** (регистр важен, но обычно не критичен)
- Это просто текст, который вы вставляете в Railway Variables
- **НЕ нужно** создавать что-то в Google Cloud Console для этого
...
3. Перейдите в **Variables** (вкладка сверху)
4. Нажмите **"+ New Variable"** (или **"Raw Editor"** для массового добавления)

### Способ 1: По одной переменной

Для каждой переменной:
- **Name:** `GOOGLE_CLIENT_ID` (точное название!)
...
- `GOOGLE_CLIENT_SECRET`
- `SESSION_SECRET`
- `ALLOWED_USERS` ← **Здесь просто напишите ваш email!**
- `CALLBACK_URL`
- `PORT`
- `NODE_ENV`

### Способ 2: Все сразу (Raw Editor)

1. Нажмите **"Raw Editor"** в Railway Variables
2. Вставьте все переменные в формате:
  ...
  SESSION_SECRET=zBbumNM4ZBRdDCO4e7GVBpbVUF9/aT+hHaQouID/TQg=
  ...
  ```
3. Замените:
   - `ваш-client-id-из-google` → Client ID из Google Cloud Console
   - `ваш-client-secret-из-google` → Client Secret из Google Cloud Console
   - `ваш-email@gmail.com` → **ваш реальный email для входа**
   - `ваш-домен.railway.app` → домен из Railway Settings
4. Нажмите **"Save"**

### Пример заполнения ALLOWED_USERS:

Если вы хотите разрешить доступ только себе:
```
ALLOWED_USERS=ivan.petrov@gmail.com
```

Если хотите разрешить нескольким людям:
```
ALLOWED_USERS=ivan.petrov@gmail.com,maria.sidorova@gmail.com
```
